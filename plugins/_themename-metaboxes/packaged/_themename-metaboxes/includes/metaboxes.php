...
add_action( 'add_meta_boxes', 'bubdrops_add_meta_box' );
function bubdrops_add_meta_box() {
  add_meta_box( 
      'bubdrops_post_metabox', 
      //title: appears top of meta setting
      'Post Settings', 
      //callback responsible for rendering the content inside the metabox
      'bubdrops_post_metabox_html', 
...
function bubdrops_post_metabox_html($post) {
  ...
  $subtitle = get_post_meta($post->ID, '_bubdrops_post_subtitle', true);
  $layout = get_post_meta($post->ID, '_bubdrops_post_layout', true);
  ...
  wp_nonce_field( 'bubdrops_update_post_metabox', 'bubdrops_update_post_nonce' );
  ...
  ?>
    <p>
        <label for="bubdrops_post_metabox_html"><?php esc_html_e( 'Post Subtitle', 'bubdrops-metaboxes' ); ?></label>
        <br />
        <!-- name will be available in the post superglobal -->
        <input class="widefat" type="text" name="bubdrops_post_subtitle_field" id="bubdrops_post_metabox_html" value="<?php echo esc_attr( $subtitle ); ?>" />
    </p>
    <p>
        <label for="bubdrops_post_layout_field"><?php esc_html_e( 'Layout', 'bubdrops-metaboxes' ); ?></label>
        <select name="bubdrops_post_layout_field" id="bubdrops_post_layout_field" class="widefat">
            <!-- selected( $selected:mixed, $current:mixed, $echo:boolean ) -->
            <option <?php selected( $layout, 'full' ); ?> value="full"><?php esc_html_e( 'Full Width', 'bubdrops-metaboxes' ); ?></option>
            <option <?php selected( $layout, 'sidebar' ); ?> value="sidebar"><?php esc_html_e( 'Post With Sidebar', 'bubdrops-metaboxes' ); ?></option>
        </select>
    </p>
  <?php
...
add_action( 'save_post', 'bubdrops_save_post_metabox', 10, 2 );
function bubdrops_save_post_metabox($post_id, $post) {
  ...
  if( !isset( $_POST['bubdrops_update_post_nonce']) || !wp_verify_nonce( $_POST['bubdrops_update_post_nonce'], 'bubdrops_update_post_metabox' )) {
    return;
  }

  if(array_key_exists('bubdrops_post_subtitle_field', $_POST)) {
    update_post_meta( 
        $post_id, 
        ...
        '_bubdrops_post_subtitle',
        //the value of what the user typed
        sanitize_text_field($_POST['bubdrops_post_subtitle_field'])
    );
  }


  if(array_key_exists('bubdrops_post_layout_field', $_POST)) {
    update_post_meta( 
        $post_id, 
        '_bubdrops_post_layout', 
        sanitize_text_field($_POST['bubdrops_post_layout_field'])
    );
  } 
